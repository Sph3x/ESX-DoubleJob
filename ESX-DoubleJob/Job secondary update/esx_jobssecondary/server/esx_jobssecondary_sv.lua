...
RegisterServerEvent('esx_jobssecondary:setCautionInCaseOfDrop')
AddEventHandler('esx_jobssecondary:setCautionInCaseOfDrop', function(caution)
  local _source = source
  local xPlayer = ESX.GetPlayerFromId(_source)
  xPlayer.set('caution', caution)
end)

RegisterServerEvent('esx_jobssecondary:giveBackCautionInCaseOfDrop')
AddEventHandler('esx_jobssecondary:giveBackCautionInCaseOfDrop', function()
  local _source = source
  local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_jobssecondary:startWork')
AddEventHandler('esx_jobssecondary:startWork', function(item)
  local source = source
  PlayersWorking[source] = true
  Work(source, item)
end)

RegisterServerEvent('esx_jobssecondary:stopWork')
AddEventHandler('esx_jobssecondary:stopWork', function()
  local source = source
  PlayersWorking[source] = false
end)

RegisterServerEvent('esx_jobssecondary:caution')
AddEventHandler('esx_jobssecondary:caution', function(cautionType, cautionAmount, spawnPoint, vehicle)
  local source = source
  local xPlayer = ESX.GetPlayerFromId(source)
  if cautionType == "take" then
    ...
    TriggerClientEvent('esx_jobssecondary:spawnJobVehicle', source, spawnPoint, vehicle)
