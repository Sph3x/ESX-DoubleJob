...
function ShowJobListingMenu(data)
	ESX.TriggerServerCallback('esx_joblistingsecondary:getJobsList', function(data)
		local elements = {}
		for i = 1, #data, 1 do
			table.insert(
				elements,
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				TriggerServerEvent('esx_joblistingsecondary:setJob2', data.current.value)
...
AddEventHandler('esx_joblistingsecondary:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if not isInJoblistingMarker and hasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_joblistingsecondary:hasExitedMarker')
		end
	end
end)
